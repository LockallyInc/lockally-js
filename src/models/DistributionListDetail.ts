/* tslint:disable */
/* eslint-disable */
/**
 * lockally control plane
 * The lockally control plane lets integrators and direct customers manage everything except actual mail data flow (which uses standard JMAP / IMAP / SMTP-submission against the data plane endpoints).  **Authentication.** All `/v1/_*` endpoints require a Bearer API key in the `Authorization` header. Keys are formatted `lk_live_<8-char-prefix>_<32-char-secret>`. Generate the first key for a tenant via `cmd/seed`; subsequent keys via `POST /v1/api-keys`.  **Errors.** Failures return [RFC 9457](https://www.rfc-editor.org/rfc/rfc9457) `application/problem+json` documents.  **Scopes.** Each endpoint requires a specific scope on the presented key. Insufficient scope returns `403` with the required scope name in `detail`. 
 *
 * The version of the OpenAPI document: 0.1.0
 * Contact: support@lockally.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface DistributionListDetail
 */
export interface DistributionListDetail {
    /**
     * 
     * @type {string}
     * @memberof DistributionListDetail
     */
    id: string;
    /**
     * 
     * @type {string}
     * @memberof DistributionListDetail
     */
    tenantId: string;
    /**
     * 
     * @type {string}
     * @memberof DistributionListDetail
     */
    listAddress: string;
    /**
     * 
     * @type {string}
     * @memberof DistributionListDetail
     */
    name?: string;
    /**
     * 
     * @type {Date}
     * @memberof DistributionListDetail
     */
    createdAt: Date;
    /**
     * 
     * @type {Array<string>}
     * @memberof DistributionListDetail
     */
    members: Array<string>;
    /**
     * 
     * @type {number}
     * @memberof DistributionListDetail
     */
    memberCount: number;
}

/**
 * Check if a given object implements the DistributionListDetail interface.
 */
export function instanceOfDistributionListDetail(value: object): value is DistributionListDetail {
    if (!('id' in value) || value['id'] === undefined) return false;
    if ((!('tenantId' in value) && !('tenant_id' in value)) || (value['tenantId'] === undefined && value['tenant_id'] === undefined)) return false;
    if ((!('listAddress' in value) && !('list_address' in value)) || (value['listAddress'] === undefined && value['list_address'] === undefined)) return false;
    if ((!('createdAt' in value) && !('created_at' in value)) || (value['createdAt'] === undefined && value['created_at'] === undefined)) return false;
    if (!('members' in value) || value['members'] === undefined) return false;
    if ((!('memberCount' in value) && !('member_count' in value)) || (value['memberCount'] === undefined && value['member_count'] === undefined)) return false;
    return true;
}

export function DistributionListDetailFromJSON(json: any): DistributionListDetail {
    return DistributionListDetailFromJSONTyped(json, false);
}

export function DistributionListDetailFromJSONTyped(json: any, ignoreDiscriminator: boolean): DistributionListDetail {
    if (json == null) {
        return json;
    }
    return {
        
        'id': json['id'],
        'tenantId': json['tenant_id'],
        'listAddress': json['list_address'],
        'name': json['name'] == null ? undefined : json['name'],
        'createdAt': (new Date(json['created_at'])),
        'members': json['members'],
        'memberCount': json['member_count'],
    };
}

export function DistributionListDetailToJSON(json: any): DistributionListDetail {
    return DistributionListDetailToJSONTyped(json, false);
}

export function DistributionListDetailToJSONTyped(value?: DistributionListDetail | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'id': value['id'],
        'tenant_id': value['tenantId'],
        'list_address': value['listAddress'],
        'name': value['name'],
        'created_at': value['createdAt'].toISOString(),
        'members': value['members'],
        'member_count': value['memberCount'],
    };
}

